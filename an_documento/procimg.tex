...
\author{
  \IEEEauthorblockN{Brandon Ascensio\IEEEauthorrefmark{1},
                    Gabriel Cortez\IEEEauthorrefmark{1},
                    Mario Umaña\IEEEauthorrefmark{1},
                    Xavier Morán\IEEEauthorrefmark{1}}
...
Este documento presenta un estudio teórico-experimental de la
...
\label{sec:marco}
%=============================================================
\subsection{Definición Matemática del Problema}
El problema fundamental a resolver en esta aplicación consiste en estimar una señal de imagen original desconocida, denotada como $f[m,n]$, a partir de una observación degradada $f_{\text{ruido}}[m,n]$. Se asume un modelo de degradación aditivo donde el ruido interfiere con la señal pura:

\begin{equation}
  f_{\text{ruido}}[m,n] = f[m,n] + \eta[m,n]
\end{equation}

donde $\eta[m,n]$ representa el ruido blanco gaussiano aditivo (AWGN). El objetivo matemático es diseñar una transformación en el dominio de la frecuencia, mediante una función de transferencia $H[u,v]$, tal que al aplicarla sobre el espectro de la imagen degradada $F_{\text{ruido}}[u,v]$, se obtenga una estimación $\hat{F}[u,v] = F_{\text{ruido}}[u,v] \cdot H[u,v]$ cuya transformada inversa $\hat{f}[m,n]$ minimice el Error Cuadrático Medio (MSE) respecto a la imagen original $f[m,n]$:

\begin{equation}
  \min_{H} \mathbb{E}\left[ \left( f[m,n] - \mathcal{F}^{-1}\{F_{\text{ruido}}[u,v] \cdot H[u,v]\} \right)^2 \right]
\end{equation}

Para efectos prácticos de esta investigación, el problema se restringe a evaluar el rendimiento de la aproximación de $H[u,v]$ utilizando un filtro gaussiano paso-bajo para maximizar la relación señal a ruido (PSNR) mitigando el impacto de $\eta[m,n]$.
...
\item \textbf{Uniformidad del filtro y ruido estructurado.}
...
\item \textbf{Suposición de estacionariedad espacial}
La Transformada Discreta de Fourier bidimensional (DFT-2D) aplicada a toda la imagen supone que las características de frecuencia son las mismas en todas sus regiones. Sin embargo, esto no ocurre cuando la imagen contiene áreas con texturas o patrones muy diferentes.

\item \textbf{Artefactos de borde por periodicidad implícita.}
La DFT supone que la imagen se repite indefinidamente. Si los bordes de la imagen son muy diferentes entre sí, pueden aparecer errores o distorsiones en el análisis de frecuencias. Para disminuir este efecto, se pueden usar ventanas bidimensionales o extender la imagen de forma simétrica antes del procesamiento.

\item \textbf{Distorsión por saturación de píxeles.}
La operación $\text{clip}(0, 255)$ post-IFFT introduce distorsión
